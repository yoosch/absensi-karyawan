...
export const columns = [
  ...
  { name: "DETAIL MASUK", uid: "detail_masuk" },
  { name: "FOTO SELFI MASUK", uid: "foto_selfi_masuk" },
  { name: "DETAIL KELUAR", uid: "detail_keluar" },
  { name: "FOTO SELFI KELUAR", uid: "foto_selfi_keluar" },
];
...
const INITIAL_VISIBLE_COLUMNS = ["tanggal", "detail_masuk", "foto_selfi_masuk", "detail_keluar", "foto_selfi_keluar"];

export default function App({ logAbsen }) {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (dateRange.tanggalMulai && dateRange.tanggalSelesai) {
        const startDate = new Date(dateRange.tanggalMulai);
        const endDate = new Date(dateRange.tanggalSelesai);
        filteredUser = filteredUser.filter((user) => {
    ...
    return filteredUser;
}, [user, filterValue, hasSearchFilter, dateRange]);


  React.useEffect(() => {
    console.log('Tanggal Mulai selected' + dateRange.tanggalMulai);
    console.log('Tanggal Selesai selected' + dateRange.tanggalSelesai);
    }, [dateRange]);
  ...
  const renderCell = React.useCallback((user, columnKey) => {
    switch (columnKey) {
      case "foto_selfi_masuk":
        const foto_masuk = user.photo_masuk_url;
        return foto_masuk?(
            <Image
      isZoomed
      alt={'absen'+user.nama+'masuk'+user.tanggal}
      src={foto_masuk}
      width={120}
    />
        ):(
            <div className="text-default-400 text-xs">Tidak ada foto</div>
        )
    case "foto_selfi_keluar":
        const foto_keluar = user.photo_keluar_url;
        return foto_keluar?(
            <Image
        isZoomed
        alt={'absen'+user.nama+'keluar'+user.tanggal}
        src={foto_keluar}
        width={120}
        />
        ):(
    ...
    case "tanggal":
        return (
            <div className="text-left text-xs text-black">{new Date(user.tanggal).toLocaleDateString()}</div>
        )
      default:
